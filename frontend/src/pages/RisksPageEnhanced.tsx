                        ...
                        filteredRisks.map((risk: any) => {
                          ...
                          return (
                            <tr key={risk.id} className="border-b hover:bg-muted/30 transition-colors">
                              ...
                              {visibleColumns.treatment && <td className="px-3 py-2.5">{risk.treatment && risk.treatment !== 'PENDING' ? <Badge variant="outline" className={cn('font-medium', { 'bg-blue-50 text-blue-700 border-blue-200': risk.treatment === 'MITIGATE', 'bg-amber-50 text-amber-700 border-amber-200': risk.treatment === 'ACCEPT', 'bg-purple-50 text-purple-700 border-purple-200': risk.treatment === 'TRANSFER', 'bg-red-50 text-red-700 border-red-200': risk.treatment === 'AVOID' })}>{risk.treatment}</Badge> : '-'}</td>}
