...
import {
  ...
  findSimilarChanges,
  askAboutChanges,
} from '../services/changeKnowledge.service.js';
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const results = await searchChanges(
      q as string,
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const results = await findSimilarChanges(
      itopId,
      organizationId as string,
      Number(limit) || 5
    );
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const answer = await askAboutChanges(question, organizationId);
